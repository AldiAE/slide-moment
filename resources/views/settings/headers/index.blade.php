                ...
                <tbody class="text-gray-600 fw-semibold">
                    @forelse ($headers as $header)
                        <tr>
                            ...
                            <td>{{ $header->slug }}</td>
                            <td class="text-end">
                                <a href="{{ route('headers.edit', $header->id) }}" class="btn btn-sm btn-light-warning me-2">
                                    Edit
                                </a>
                                <form action="{{ route('headers.destroy', $header->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-light-danger" onclick="return confirm('Delete this header?')">
                                        Delete
                                    </button>
                                </form>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted">No headers found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
