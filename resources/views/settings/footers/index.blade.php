                ...
                <tbody class="text-gray-600 fw-semibold">
                    @forelse ($footers as $footer)
                        <tr>
                            ...
                            <td class="text-end">
                                <a href="{{ route('footers.edit', $footer->id) }}" class="btn btn-sm btn-light-warning me-2">Edit</a>
                                <form action="{{ route('footers.destroy', $footer->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-light-danger" onclick="return confirm('Delete this footer?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center text-muted">No footers found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
